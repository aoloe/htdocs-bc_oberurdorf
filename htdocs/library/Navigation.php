...
class Navigation {
    ...
    public function get_rendered($site_structure, $structure_url) {
        $result = "";
        $navigation = array();
        ...
        foreach ($site_structure as $key => $value) {
            ...
            if (!empty($value) && array_key_exists('navigation', $value) && $value['navigation']) {
                $navigation[] = array (
                    'href' => array_key_exists('url', $value['navigation']) ? $value['navigation']['url'] : $key,
                    'label' => array_key_exists('label', $value['navigation']) ? $value['navigation']['label'] : $key,
        ...
        $result = $template->fetch('template/navigation.php');
        return $result;
    }
